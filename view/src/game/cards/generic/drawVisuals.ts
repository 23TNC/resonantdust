...
/** Map the wasm `PrimNode` JSON onto the client `VisualNode[]`. `texture` is the
 *  resolved STEM string from `^r2` (empty → no art); `text` is a locale key. */
function mapPrims(nodes: PrimNodeJson[]): PrimList {
  ...
  return nodes.map((n) => ({
    ...
    alpha: n.alpha,
    tint: n.tint,
    texture: n.texture != null && n.texture !== "" ? n.texture : null,
