...
function renderAndPack(
  ...
  g.destroy();
  // Colour-only fill — no normal source, so the normal page is untouched.
  const packed = textures.pack(rt).albedo;
  rt.destroy(true);
  return new Texture({
 ...
 *  `worldHexRadius()` (the largest hex display size) so it's never upscaled,
