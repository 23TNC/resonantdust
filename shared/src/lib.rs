...
mod tests {
    ...
    fn loads_and_matches_a_recipe() {
        ...
        let plan = c
            .match_recipe(&[("slot.1.0".into(), Card { def_id: id, stock: vec![], stock_raw: 0, ..Default::default() })], "use_corpus")
            .expect("matched");
        assert!(plan.matched);
        let out = c
            .plan_recipe(&[("slot.1.0".into(), Card { def_id: id, stock: vec![], stock_raw: 0, ..Default::default() })], "use_corpus")
            .expect("planned");
